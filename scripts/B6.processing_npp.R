# ------------------------------------------------------------------------------
# Program Name: B6.processing_npp.R
# Authors: Leeya Pressburger
# Date Last Modified: March 2022
# Program Purpose: Processing CMIP6 npp data
# Inputs: csv files generated by A6.npp.py
# Outputs: One csv file with annual npp values for all CMIP6 
...
npp <- left_join(data, years, by = "rownum")
...
replace_year <- ifelse(is.na(npp$new_year), 
                       npp$year, 
                       npp$new_year)

# Combine with original dataframe
npp$year <- replace_year

# Get rid of unnecessary columns
npp <- npp %>% select(c(-new_year, -File.y, -X1))
...
npp_low <- filter(npp, value < 10)
low_models <- unique(npp_low$model)
npp <- filter(npp, !model %in% low_models)

# Plot npp over time
npp_plot <- ggplot(npp, aes(year, value, color = model, 
                          group = paste0(model, experiment, ensemble))) +
  geom_line() +
  facet_wrap(~experiment, scales = "free") +
  labs(x = "Year",
       y = "Pg C",
       title = "Total NPP on Land as Carbon Mass Flux") +
  theme_minimal()
...
# Save outputs to csv
write.csv(output, "./outputs/CMIP6_annual_npp_land.csv", row.names = FALSE)
